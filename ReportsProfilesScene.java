...
public class ReportsProfilesScene implements Sceneable {

    private final AdminSceneNavigator navigator;
    private final static QueryExecutor queryExecutor = new QueryExecutor();

    public ReportsProfilesScene(AdminSceneNavigator navigator) {
    ...
    public Scene getScene() {
        ...
        List<String> userReportIds = fetchReportIds();
        ListView<String> reportFileView = new ListView<>();
        reportFileView.getItems().addAll(userReportIds);
    ...
    private void showReportDetails(String reportId, TextArea reportDetailsArea) {
        ...
        try (Connection conn = queryExecutor.getAppAdminConnection();
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) {
                    String user = rs.getString("reported_user");
                    ...
                    String reportDetails = String.format(
                            "User: %s\nReason: %s\nDate: %s\nAdmin Response: %s",
                            user, reason, date, adminResponse);
    ...
    private void updateResponse(String reportId, String responseText) {
        String updateQuery = "UPDATE user_reports SET admin_response = ? WHERE report_id = ?";
